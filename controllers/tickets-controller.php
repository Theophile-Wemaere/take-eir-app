    ...
    if (isset($_GET["action"])) {
        switch ($_GET["action"]) {
            ...
            case "tickets":
                ...
                if ($_GET["search"] != "" and $_GET["search"] != "select") {
                    $query = $query . " AND tags.id_tag = :id_tag";
                }
                ...
                if($_GET["search"] != "" and $_GET["search"] != "select")
                {
                    $results = $_DB->execute($query, $admin ? ["id_tag" => $_GET["search"]] : [
                        "id" => $_SESSION["id"],
                        "id_tag" => $_GET["search"]
                    ])->fetchAll();
                } else {
                    $results = $_DB->execute($query, $admin ? null : [
                        "id" => $_SESSION["id"]
                    ])->fetchAll();
                }
                echo json_encode(count($results) == 0 ? null : $results);
